...
def calculate_remains(s, product_id):
    stmt = select(Stock_movement.quantity, Stock_movement.movement_type).where(Stock_movement.product_id==product_id)
...
def get_quality_by_product():
    with sesion_local() as s:
        stmt = select(Product.name, Product.id)
        res = s.execute(stmt).all()
        print(res)
        for el in res:
            print(el[0], ': ', calculate_remains(s, el[1]))


def running_out_products():
    with sesion_local() as s:
        stmt = select(Product.name, Product.id, Product.min_quantity)
        res = s.execute(stmt).all()
        for el in res:
            if calculate_remains(s, el[1]) <= el[2]:
                print(f'{el[0]}\nОстаток: {calculate_remains(s, el[1])}\n Минимальный остаток: {el[2]}')

def total_purchase_sum():
    with sesion_local() as s:
        stmt = select(Product.name, Product.id, Product.purchase_price)
        res = s.execute(stmt).all()
        for el in res:
            print(f'{el[0]}:\nОстаток: {calculate_remains(s, el[1])}\nЗакупочная цена:{el[2]}\nСтоимость остатка: {calculate_remains(s, el[1])*el[2]}')
    
def total_selling_price():
    with sesion_local() as s:
        stmt = select(Product.name, Product.id, Product.selling_price)
        res = s.execute(stmt).all()
        for el in res:
            print(f'{el[0]}:\nОстаток: {calculate_remains(s, el[1])}\nПродажная цена:{el[2]}\nПотецниальная выручка: {calculate_remains(s, el[1])*el[2]}')


def total_revenue():
    with sesion_local() as s:
        stmt = select(Product.name, Product.id, Product.purchase_price, Product.selling_price)
        res = s.execute(stmt).all()
        for el in res:
            print(f'{el[0]}:\nОстаток: {calculate_remains(s, el[1])}\nЗакупочная цена: {el[2]}\nПродажная цена:{el[3]}\nПотецниальная выручка: {calculate_remains(s, el[1])*(el[3]-el[2])}')
